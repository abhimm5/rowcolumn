...
var Rect = class {
  constructor(x, y, w, h2, styles, isSkipped, customSize, spread = { t: 0, r: 0, b: 0, l: 0 }) {
    this.x = x;
    this.y = y;
    this.w = w;
    this.h = h2;
...
var Engine = {
  ...
  calc(n, x, y, w, h2, leaves, pSkip, offRules, activePropRules, activeSpreadRules) {
    if (!n) return;
    ...
    if (!isLayoutNode || !n.children.length) {
      ...
      leaves.push(new Rect(x, y, w, h2, finalStyles, finalSkip, customSize, finalSpread));
      return;
    }
    let tw = 0;
    ...
    n.children.forEach((c, i) => {
      ...
      if (n.type === "col") {
        ...
        this.calc(c, pos, y, cw, h2, leaves, skip, offs, nextPropRules, nextSpreadRules);
        pos += cw;
      } else {
...
// src/vue.ts
var Layout = (0, import_vue.defineComponent)({
  name: "Layout",
  props: {
    layout: { type: [String, Object], required: true },
    "layout-sm": { type: [String, Object] },
    "layout-md": { type: [String, Object] },
    "layout-lg": { type: [String, Object] }
  },
  setup(props, { slots, attrs }) {
    ...
    (0, import_vue.onUpdated)(() => update());
    (0, import_vue.onUnmounted)(() => window.removeEventListener("resize", update));
    return () => {
      const domAttrs = { ...attrs };
